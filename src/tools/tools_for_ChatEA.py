...
class NeighborGenerator(object):
    ...
    def load_candidates(self, cand_file='cand'):
        ...
        for eid, data in origin_cand.items():
            eid = int(eid)
            ref[eid] = data['ref'] if 'ref' in data else None
            rank[eid] = data['ground_rank']  if 'ground_rank' in data else None       # ranks from method based on embeddings
            cand[eid] = data['candidates']   if 'candidates' in data else None       # {ent_id: [cand_id_1, cand_id_2, ..., cand_id_20], ...}
            cand_score[eid] = data['cand_sims']  if 'cand_sims' in data else None   # similarity scores from method based on embeddings
