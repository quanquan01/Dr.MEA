...
def load_img_features(ent_num, file_dir):
    ...
    if "V1" in file_dir:
        ...
        img_vec_path = "../data/pkls/FBYG15K_id_img_feature_dict.pkl"
    else:
...
def load_desc_features(ent_num, file_dir, flag):
    ...
    if "V1" in file_dir:
        ...
        filename = os.path.split(file_dir)[-1].upper()
        desc_vec_path = "../data/pkls/FBYG15K_id_desc_feature_dict.pkl"
    else:
        split = file_dir.split("/")[-1]
...
def load_attr_add_features(ent_num, file_dir, flag):
    ...
    if "V1" in file_dir:
        ...
        filename = os.path.split(file_dir)[-1].upper()
        attr_vec_path = "../data/pkls/FBYG15K_id_attr_feature_dict.pkl"
    else:
        split = file_dir.split("/")[-1]
...
class MCLEA:
    ...
    def parse_options(parser):
        parser.add_argument("--file_dir", type=str,
                            default="../data/FBYG15K", required=False,
                            help="input dataset file directory, ('data/DBP15K/zh_en', 'data/DWY100K/dbp_wd')")
        parser.add_argument("--rate", type=float, default=0.8, help="training set rate")
        ...
        parser.add_argument("--bsize", type=int, default=3500, help="batch size")
    ...
    def init_data(self):
        ...
        # 指定要保存的文件路径
        file_path = '../data/FBYG15K/ref_pairs'
        ...
        with torch.no_grad():
            ...
            if "100" in self.args.file_dir:
                ...
                if epoch + 1 == self.args.epochs:
                    ...
                    # 指定保存的文件路径
                    output_file_path = '../data/FBYG15K/candidates/similarity_matrix.npy'
