...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, args,
                 ent_num,
                 img_feature_dim,
                 attr_add_feature_dim=None,
                 desc_feature_dim=None,
        ...
        desc_dim=self.args.desc_dim
        char_dim = self.args.char_dim
        attr_add_dim=self.args.attr_add_dim
        ...
        self.attr_add_fc = nn.Linear(attr_add_feature_dim, attr_add_dim)
        self.desc_fc = nn.Linear(desc_feature_dim, attr_dim)
        ...
        if self.use_project_head:
            ...
            self.desc_pro=ProjectionHead(desc_dim, desc_dim, desc_dim, dropout)
            self.attr_add_pro=ProjectionHead(attr_add_dim, attr_add_dim, attr_add_dim, dropout)
    ...
    def forward(self,
        ...
        if self.args.w_name:
            ...
            name_emb = None
        if self.args.w_attr_add:
            attr_add_emb = self.attr_add_fc(attr_add_features)
        else:
            attr_add_emb = None
        if self.args.w_desc:
        ...
        if self.use_project_head:
            ...
            desc_emb = self.desc_pro(desc_emb)
            attr_add_emb = self.attr_add_pro(attr_add_emb)
            pass
        if name_emb:
            joint_emb = self.fusion([img_emb, att_emb, rel_emb, gph_emb, name_emb, attr_add_emb,desc_emb])
            return gph_emb, img_emb, rel_emb, att_emb, name_emb, attr_add_emb, desc_emb, joint_emb
        else:
            joint_emb=self.fusion([img_emb, att_emb, rel_emb, gph_emb,attr_add_emb,desc_emb])
            # joint_emb=self.fusion([img_emb, att_emb, rel_emb, gph_emb,attr_add_emb])
            return gph_emb,img_emb,rel_emb,att_emb,attr_add_emb,desc_emb,joint_emb
