...
def load_img(e_num, path):
    ...
    # 返回包含实体 ID 和其嵌入的新字典
    formatted_img_dict = {i: img_dict[i] if i in img_dict else np.random.normal(mean, std, mean.shape[0]) for i in
                          range(e_num)}
    return img_embd

def load_desc(e_num, path,flag):
    desc_dict = pickle.load(open(path, "rb"))
    # init unknown img vector with mean and std deviation of the known's
    desc_np = np.array(list(desc_dict.values()))
    mean = np.mean(desc_np, axis=0)
    std = np.std(desc_np, axis=0)
    mean = np.squeeze(mean)
    std = np.squeeze(std)
    # img_embd = np.array([np.zeros_like(img_dict[0]) for i in range(e_num)]) # no image
    # img_embd = np.array([img_dict[i] if i in img_dict else np.zeros_like(img_dict[0]) for i in range(e_num)])
    desc_embd = np.array(
        [desc_dict[i] if i in desc_dict else np.random.normal(mean, std, mean.shape[0]) for i in range(e_num)])
    print("%.2f%% entities have desc" % (100 * len(desc_dict) / e_num))

    formatted_desc_dict={i:desc_dict[i] if i in desc_dict else np.random.normal(mean,std,mean.shape[0]) for i in range(e_num)}
    if flag:
        return formatted_desc_dict
    else:return desc_embd


def load_attr_add(e_num, path,flag):
    attr_dict = pickle.load(open(path, "rb"))
    # init unknown img vector with mean and std deviation of the known's
    attr_np = np.array(list(attr_dict.values()))
    mean = np.mean(attr_np, axis=0)
    std = np.std(attr_np, axis=0)
    mean = np.squeeze(mean)
    std = np.squeeze(std)
    attr_embd = np.array(
        [attr_dict[i] if i in attr_dict else np.random.normal(mean, std, mean.shape[0]) for i in range(e_num)])
    print("%.2f%% entities have desc" % (100 * len(attr_dict) / e_num))

    formatted_attr_dict={i:attr_dict[i] if i in attr_dict else np.random.normal(mean,std,mean.shape[0]) for i in range(e_num)}
    if flag:
        return formatted_attr_dict
    else:return attr_embd
