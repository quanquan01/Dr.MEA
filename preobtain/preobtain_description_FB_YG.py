...
def get_description(idx, ng: NeighborGenerator, entities, neigh=0, max_tokens=50):
    description = {}
    output_file_path = r'D:\jinquanxin\LLM4EA\data\FBYG15K\candidates\description1.json'
    i = 1
    for eid in tqdm(entities, desc=f"{idx:2d}", position=idx):
        desc={}
        ent = ng.get_neighbors(eid, neigh)
...
def generate_entity_description(data, data_dir, cand_file="cand", desc_file="description", ent=0, neigh=0,
                                max_tokens=100, use_time=True):
    ng = NeighborGenerator(data=data, data_dir=data_dir, use_time=use_time, use_desc=False, cand_file=cand_file,
                           desc_file=desc_file)
...
description = generate_entity_description(data=args.data, data_dir=args.data_dir, cand_file=args.cand_file,
                                          desc_file=args.desc_file, ent=args.ent, neigh=args.neigh,
                                          max_tokens=args.max_tokens, use_time=use_time)
